...
table_summary %>% 
  kableExtra::kbl(booktabs = T, 
      align = "r",
      caption = stringr::str_c(target_name, "の各種数値の推移")) %>%
...
pre_target_data %>%
  ...
  # 以下は同じ処理
  ggtitle(paste0(target_name,"の推定土地単価の箱ひげ図と平均の推移"))+
  scale_y_continuous(labels =
                       function(v){
                         retiex::style_yen(v, 0, "円／㎡")
                       }) +
...
pre_target_data %>%
  ...
  ggtitle(paste0(target_name,"の推定土地単価のバイオリンプロット"))+
  scale_y_continuous(labels =
                       function(v){
                         retiex::style_yen(v, 0, "円／㎡")
                       }) +
...
target_data %>%
  ...
  # 以下は同じ処理
  ggtitle(paste0(target_name,"に於ける推定土地単価平均の増減比の推移"))+
  xlab(label = "取引時点") +
  ylab(label = "増減比") +
  scale_y_continuous(labels =
                       function(v){
...
target_data %>%
  ...
  # 以下は同じ処理
  ggtitle(paste0(target_name,"の推定土地単価の移動平均の増減比の推移"))+
  xlab(label = "取引時点") +
  ylab(label = "増減比") +
  scale_y_continuous(labels =
                       function(v){
